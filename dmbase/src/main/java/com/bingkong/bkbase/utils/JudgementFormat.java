...
public class JudgementFormat {
    ...
    static String Y = "^([2-4]{1})([0-9]{1}[0-9]{1})[0-9]{7}$";
...
//    static String DX = "^[1]{1}(([3]{1}[3]{1})|([5]{1}[3]{1})|([8]{1}[09]{1}))[0-9]{8}$";
    ...
    public JudgementFormat(String mobPhnNum) {
        this.mobPhnNum = mobPhnNum;
        LogUtils.logd("tool", mobPhnNum);
    }
...
//            }
        ...
        LogUtils.logd("TelNumMatch", "flag" + flag);
        return flag;
    }

    //手机号码的有效性验证
    public static boolean isValidPhoneNumber(String number) {
        return true;//return true directly before it is correct.
        /*
        boolean flag=false;
        if(number.length()==10 && (number.matches(Y)))
        {
            flag=true;
        }
        return flag;
    ...
    public static boolean isEmail(String email) {
        String patternString = "[\\w!#$%&'*+/=?^_`{|}~-]+(?:\\.[\\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\\w](?:[\\w-]*[\\w])?\\.)+[\\w](?:[\\w-]*[\\w])?";
